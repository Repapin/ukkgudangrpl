...
<div class="container mt-5">
    ...
    <form method="GET">
        <input type="text" name="search" class="form-control" placeholder="Cari Nama Barang" required>
        ...
        <ul class="list-group">
            <?php foreach ($search_result as $item): ?>
                <li class="list-group-item">
                    <?= htmlspecialchars($item['nama_barang']) ?> - Rp<?= number_format($item['harga_barang'], 2, ',', '.') ?>
                </li>
            <?php endforeach; ?>
        </ul>
    <?php elseif (isset($_GET['search'])): ?>
        <p class="text-danger mt-3">Barang tidak ditemukan.</p>
    <?php endif; ?>
</div>
</body>
</html>